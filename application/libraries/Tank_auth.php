...
class Tank_auth
{
    ...
    function login($login, $password, $remember, $login_by_username, $login_by_email)
    {
        if ((strlen($login) > 0) AND (strlen($password) > 0)) {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) {    // login ok
                ...
                if ($hasher->CheckPassword($password, $user->user_password)) {        // password ok

                    if ($user->user_banned == 1) {                                    // fail - banned
                        $this->error = array('banned' => $user->user_ban_reason);

                    } else {
                        $this->ci->session->set_userdata(array(
                                'user_id'    => $user->user_id,
                                'username'    => $user->user_login,
                                'status'    => ($user->user_activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
                                'user_role_id'    => $user->user_role_id,
                        ));

                        if ($user->user_activated == 0) {                            // fail - not activated
    ...
    {
    	//print_r($activated."    ");
    	//print_r($this->ci->session->userdata('status')); die();
     ...
     * @return    array
     */
    function create_user($data = array(), $email_activation)
    {		
        if((strlen($data['user_login']) > 0) AND !$this->ci->users->is_login_available($data['user_login'])) {
            $this->error = array('username' => 'auth_username_in_use');
 
        } elseif (!$this->ci->users->is_email_available($data['user_email'])) {
            ...
            $hasher = new PasswordHash(
                    $this->ci->config->item('phpass_hash_strength', 'tank_auth'),
                    $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
            $hashed_password = $hasher->HashPassword($data['user_password']);

            $data['user_password'] = $hashed_password;
            $data['user_last_ip'] = $this->ci->input->ip_address();
    ...
    function change_email($email)
    {
        ...
        if (!is_null($user = $this->ci->users->get_user_by_id($user_id, FALSE))) {
            ...
            if (strtolower($user->user_email) == strtolower($email)) {        // leave activation key as is
                $data['new_email_key'] = $user->user_new_email_key;
                return $data;
    ...
    function change_password($old_pass, $new_pass)
    {
        ...
        if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
            ...
            $hasher = new PasswordHash(
                    $this->ci->config->item('phpass_hash_strength', 'tank_auth'),
                    $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
            if ($hasher->CheckPassword($old_pass, $user->user_password)) {            // success
    ...
    function set_new_email($new_email, $password)
    {
        ...
        if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
            ...
            $hasher = new PasswordHash(
                    $this->ci->config->item('phpass_hash_strength', 'tank_auth'),
                    $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
            if ($hasher->CheckPassword($password, $user->user_password)) {            // success
                ...
                } elseif ($user->user_new_email == $new_email) {        // leave email key as is
                    $data['new_email_key'] = $user->user_new_email_key;
                    return $data;
